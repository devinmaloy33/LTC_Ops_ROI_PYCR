...
export async function GET(request: NextRequest) {
  ...
  const result = await getD1().prepare(`
    SELECT
      ...
      COALESCE((SELECT COUNT(*) FROM voice_calls v WHERE v.ccn = f.ccn AND v.appointment_status = 'confirmed'), 0) AS confirmed_appointment_count,
  ...
  return NextResponse.json({
    summary: {
      ...
      appointmentsConfirmed: rows.reduce((sum, row) => sum + row.confirmedAppointmentCount, 0),
    },
...
function normalizeFacilityRow(row: Record<string, unknown>) {
  return {
    ...
    confirmedAppointmentCount: Number(row.confirmed_appointment_count || 0),
