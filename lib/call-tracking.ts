...
export async function persistConversationUpdate(input: {
  ...
  `).bind(
    ...
    terminal ? 1 : 0,
    now,
    normalized.status === 'failed' ? 1 : 0,
    now,
    input.callId,
  ).run();

  // ── Opt-out: add to DNC list ──────────────────────────────────────────────
  if (extracted.optOut) {
    const call = await input.db
      .prepare('SELECT ccn, phone_number FROM voice_calls WHERE id = ?')
      .bind(input.callId)
      .first<{ ccn: string | null; phone_number: string | null }>();
    if (call?.phone_number) {
      await input.db.prepare(`
        INSERT OR IGNORE INTO do_not_call_numbers
          (phone_number, ccn, source_call_id, reason, created_by, created_at)
        VALUES (?, ?, ?, ?, ?, ?)
      `).bind(
        String(call.phone_number),
        call.ccn ? String(call.ccn) : null,
        input.callId,
        'Recipient requested no further calls during the conversation.',
        `elevenlabs_${input.source}`,
        now,
      ).run();
    }
  }
  ...
  // Fires only on terminal status AND confirmed appointment AND prospect has
  // an email address captured. Non-blocking — failure is logged, not thrown.
  if (terminal && appointmentStatus === 'confirmed') {
    const call = await input.db
      .prepare(`
        SELECT
          ccn, facility_name,
          COALESCE(captured_contact_name, known_contact_name) AS contact_name,
          COALESCE(captured_contact_email, known_contact_email) AS contact_email,
          preferred_day, preferred_time_window, preferred_timezone
        FROM voice_calls
        LEFT JOIN facilities ON voice_calls.ccn = facilities.ccn
        WHERE voice_calls.id = ?
      `)
      .bind(input.callId)
      .first<{
        ccn: string | null;
    ...
    if (call?.contact_email) {
      try {
        const booking = await bookAppointmentFromCallOutcome({
          preferredDay: call.preferred_day,
          preferredTimeWindow: call.preferred_time_window,
          preferredTimezone: call.preferred_timezone,
          inviteeName: call.contact_name ?? 'Valued Contact',
          inviteeEmail: call.contact_email,
          facilityName: call.facility_name ?? 'the facility',
          callRecordId: input.callId,
        });

        if (booking.ok) {
          // Store the booking URL back on the call record
          await input.db.prepare(`
            UPDATE voice_calls
            SET appointment_details = COALESCE(appointment_details || ' | Calendly: ' || ?, ?),
                updated_at = ?
            WHERE id = ?
          `).bind(
            booking.bookingUrl,
            `Calendly: ${booking.bookingUrl}`,
            Date.now(),
            input.callId,
          ).run();
        } else {
          // Log the failure reason into outcome_notes — does not break the call record
          await input.db.prepare(`
            UPDATE voice_calls
            SET outcome_notes = 'Calendly booking attempted but failed: ' || ?,
                updated_at = ?
            WHERE id = ?
          `).bind(booking.reason, Date.now(), input.callId).run();
        }
      } catch (err) {
        // Truly unexpected error — log and continue, never throw
        await input.db.prepare(`
          UPDATE voice_calls
          SET outcome_notes = 'Calendly booking exception: ' || ?,
              updated_at = ?
          WHERE id = ?
        `).bind(String(err), Date.now(), input.callId).run();
      }
    }
  }
...
export function extractOutcome(dataCollection: Record<string, unknown>) {
  ...
  if (['booked', 'scheduled', 'confirmed'].includes(bookingStatus)) appointmentStatus = 'confirmed';
    ...
    ['pending_confirmation', 'tool_failed'].includes(bookingStatus) ||
    ['accepted', 'tentative'].includes(interest)
