...
import {
  Activity, ArrowLeft, Building2, Check, CheckCircle2, Clipboard, Clock3,
  Download, FileText, Loader2, LogOut, Mail, Phone, RefreshCw, Search,
...
type FacilityListResponse = {
  summary: { facilities: number; complete: number; needsCompletion: number; downloaded: number };
...
export default function AdminDashboard({ userName, userEmail, signOutPath }: { userName: string; userEmail: string; signOutPath: string }) {
  ...
  const summary = data?.summary || { facilities: 0, complete: 0, needsCompletion: 0, downloaded: 0 };
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[950px] text-left text-xs">
              <thead className="bg-slate-50 text-[10px] uppercase tracking-wider text-paycor-grey">
                <tr><th className="px-5 py-3">Facility</th><th className="px-4 py-3">CMS</th><th className="px-4 py-3">Progress</th><th className="px-4 py-3">Missing</th><th className="px-4 py-3">Lookups</th><th className="px-4 py-3">Downloads</th><th className="px-5 py-3">Last activity</th></tr>
              </thead>
              <tbody>
                {loading && <tr><td colSpan={7} className="px-5 py-12 text-center text-paycor-medium-grey"><Loader2 className="mx-auto mb-2 h-5 w-5 animate-spin" />Loading activity…</td></tr>}
                {!loading && data?.facilities.length === 0 && <tr><td colSpan={7} className="px-5 py-12 text-center text-paycor-medium-grey">No tracked facilities match these filters.</td></tr>}
